...
def main(args):
    ...
    list_geom_info = [
        os.path.join(args.data_dir, "geometry_info", f"{fn}.json") for fn in list_frames
    ]
    list_labels = [
        os.path.join(args.data_dir, "labels", "gt", f"{fn}.npz") for fn in list_frames
    ]
    ...
    if (
        np.sum([os.path.isfile(fn) for fn in list_geom_info])
        == list_geom_info.__len__()
    ):
...
def get_argparse():
    ...
    parser.add_argument("-v", action="store_true", help=f"Explicit list of failures.")
