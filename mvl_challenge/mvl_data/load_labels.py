...
def main(args):
    ...
    for list_ly in mvl.iter_list_ly():
        for ly in list_ly:
            phi_coords = load_gt_label(
                os.path.join(args.scene_dir, "labels", "gt", f"{ly.idx}.npz")
            )
            img = ly.get_rgb()
            draw_boundaries_phi_coords(img, phi_coords=phi_coords)

            plot_image(image=img, caption=ly.idx + " - GT labels")
