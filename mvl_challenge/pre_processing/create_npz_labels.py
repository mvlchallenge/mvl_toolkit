...
from mvl_challenge.utils.spherical_utils import (
    xyz2uv,
    uv2phi_coords,
    phi_coords2uv,
    phi_coords2xyz,
...
from mvl_challenge.utils.geometry_utils import (
    tum_pose2matrix44,
    extend_array_to_homogeneous,
)
...
from mvl_challenge.pre_processing.create_scene_room_idx_list import (
    save_scene_list_from_mvl_directory,
)
...
def save_phi_bound(args, list_corners, scene_room_idx_list):
    ...
    for sc_rm_idx in scene_room_idx_list:
        ...
        cam_pose = tum_pose2matrix44(
            [-1] + geom_data["translation"] + geom_data["quaternion"]
        )

        # ! Transform into camera coordinates
        local_xyz_ceil = np.linalg.inv(cam_pose)[:3, :] @ extend_array_to_homogeneous(
            bound_ceil
        )
        local_xyz_floor = np.linalg.inv(cam_pose)[:3, :] @ extend_array_to_homogeneous(
            bound_floor
        )
        ...
        img = imread(
            Path(args.geom_info_dir).parent.__str__() + f"/img/{scene_room_idx}.jpg"
        )
        ...
        img = add_caption_to_image(image=img, caption="mvl-challenge " + scene_room_idx)
...
def main(args):
    ...
    all_rooms = np.unique(
        [get_scene_room_from_scene_room_idx(room) for room in scene_room_idx_list]
    ).tolist()
    ...
    for scene in tqdm(scene_rooms, desc="Loading scenes..."):
        ...
        list_room_per_scene = sorted(
            list_room_per_scene, key=lambda x: int(x.split("room")[-1])
        )
        for room_name in list_room_per_scene:
            ...
            if os.path.exists(mvl_labels_fn):
                ...
                save_phi_bound(
                    args,
                    list_corners,
                    [g for g in scene_room_idx_list if room_name in g],
                )
...
def get_argparse():
    desc = (
        "This script creates the npy files from the mvl-annotation. "
