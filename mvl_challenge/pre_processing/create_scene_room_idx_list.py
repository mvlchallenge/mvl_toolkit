...
def get_list_rooms_idx_from_metadata(metadata_filename):
    ...
    for room in list_rooms:
        ...
        [list_room_idx.append(f"{room.replace('.', '')}_{kf}") for kf in list_kf]
...
def get_list_rooms_idx_from_mvl_labels(mvl_labels_filename):
    ...
    for room, list_kf in enumerate(mvl_data["list_kf"]):
        [list_room_idx.append(f"room{room}_{kf}") for kf in list_kf]
    return list_room_idx
...
def scene_list_from_rgbd_dataset(args):
    ...
    list_rooms = np.unique(
        [get_scene_room_from_scene_room_idx(r) for r in list_scene_room_idx]
    ).tolist()
...
def get_argparse():
    desc = (
        "This script reads a RGBD dataset directory to create and saves a `scene_room_idx` file. "
