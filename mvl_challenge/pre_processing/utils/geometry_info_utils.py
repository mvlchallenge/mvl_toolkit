...
from mvl_challenge.pre_processing.utils.camera_height_utils import (
    estimate_camera_height,
)
from mvl_challenge.pre_processing.utils.camera_height_per_rooms import (
    estimate_cam_height_per_room,
)
...
def get_geometry_info(dt: RGBD_Dataset):
    ...
    for room_name in cam_h_dict.keys():
        ...
        list_idx = sorted(
            [get_idx_from_scene_room_idx(fr_names) for fr_names in list_fr_names]
        )
        list_fr_room = sorted(
            [fr for fr in list_fr if fr.idx in list_idx], key=lambda x: x.idx
        )

        # ! List geometry info per fr
        list_geom_info.append(
            [
                {
                    f"{room_name}_{fr.idx}": {
...
def get_argparse():
    desc = (
        "This script computes the geometry information per frame from a given RGBD dataset. "
