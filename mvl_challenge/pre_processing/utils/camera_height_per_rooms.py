...
def estimate_cam_height_per_room(cfg, dt: MP3D_FPE):
    ...
    for list_fr in dt.iter_rooms_scenes():
        ...
        cam_height_dict[room_name] = dict(cam_h_rc=cam_h_rc, cam_h_wc=cam_h_wc)
    [
        logging.info(
            f"Room: {r}\tcam_h(r): {d['cam_h_rc']:2.3f}\tcam_h(w): {d['cam_h_wc']:2.3f}"
        )
        for r, d in cam_height_dict.items()
    ]
