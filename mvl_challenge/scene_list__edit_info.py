...
def split_scene_list(scene_data, split_list):
    ...
    for split, scenes in split_list.items():
        ...
        if list(new_scene_data[split].keys()).__len__() == 0:
            new_scene_data[split] = {
                k: v for k, v in scene_data.items() if k.split("_")[0] in scenes
            }
            if list(new_scene_data[split].keys()).__len__() == 0:
                new_scene_data[split] = {
                    k: v for k, v in scene_data.items() if k.split("_room")[0] in scenes
                }
...
def main(args):
    if args.r > 0:
        ...
        for key, data in new_scene_data.items():
            fn = os.path.join(
                os.path.dirname(args.scene_list), f"{Path(args.o).stem}__{key}.json"
            )
...
def get_argparse():
    ...
    parser.add_argument(
        "-o", default="pruned_scene_list", type=str, help="Output scene_list name."
    )
    ...
    parser.add_argument("-r", default=-1, type=float, help="Random sampling.")

    parser.add_argument(
        "--prune", type=int, default=-1, help="Max number of fr per room."
    )

    parser.add_argument(
        "-v", action="store_true", help="To visualize info without creating a file"
    )
