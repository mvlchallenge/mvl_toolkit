...
def check_geo_info_and_img(mvl_dir, list_scenes):
    ...
    geo_info_fn = [
        os.path.join(mvl_dir, "geometry_info", f"{sc}.json") for sc in list_scenes
    ]
    ...
    if np.sum(geo_info_check) != geo_info_check.__len__():
        [
            ...
            for ckpt, fn in zip(geo_info_check, geo_info_fn)
            if not ckpt
        ]
        output = False

    if np.sum(img_fn_check) != img_fn_check.__len__():
        [print(f"Not found {fn}") for ckpt, fn in zip(img_fn_check, img_fn) if not ckpt]
        output = False
...
def check_mvl_dataset(args):
    ...
    check = [
        ...
        for scene_list in tqdm(
            data_scene.values(), desc=f"loading and checking {args.scene_dir}"
        )
    ]
