    ...
    for scene in list_scenes:
        ...
        with zipfile.ZipFile(file=zip_filename, mode="w") as zf:
            ...
            [
                ...
                for fn in tqdm(list_arc_fn, desc=f"zipping {os.path.join(scene, fn)}")
            ]


def get_list_frames(scene_path):
    key_fr_fn = get_files_given_a_pattern(
        data_dir=scene_path,
...
def zip_rgb_files(list_scenes, args):
    for scene in list_scenes:
        ...
        with zipfile.ZipFile(file=zip_filename, mode="w") as zf:
            ...
            [
                zf.write(
                    os.path.join(scene_dir, "rgb", f"{fn}.png"),
                    compress_type=zipfile.ZIP_STORED,
                    arcname=os.path.join(scene, "rgb", f"{fn}.png"),
                )
                for fn in tqdm(list_fn, desc=f"zipping {os.path.join(scene, 'rgb')}")
            ]
...
def zip_depth_files(list_scenes, args):
    for scene in list_scenes:
        ...
        with zipfile.ZipFile(file=zip_filename, mode="w") as zf:
            ...
            [
                zf.write(
                    os.path.join(scene_dir, "depth", "tiff", f"{fn}.tiff"),
                    compress_type=zipfile.ZIP_STORED,
                    arcname=os.path.join(scene, "depth", "tiff", f"{fn}.tiff"),
                )
                for fn in tqdm(list_fn, desc=f"zipping {os.path.join(scene, 'depth')}")
            ]
