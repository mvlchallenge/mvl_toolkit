...
def zip_mvl_data(args):
    ...
    for room, scene_list in data_scene.items():
        ...
        with zipfile.ZipFile(file=zip_filename, mode="w") as zf:
            geo_info_fn = [
                os.path.join(args.scene_dir, "geometry_info", f"{sc}.json")
                for sc in scene_list
            ]
            img_fn = [
                os.path.join(args.scene_dir, "img", f"{sc}.jpg") for sc in scene_list
            ]
    ...
    copyfile(
        args.scene_list,
        os.path.join(args.output_dir, os.path.split(args.scene_list)[-1]),
    )
...
def zip_mvl_labels(args):
    ...
    for room, scene_list in data_scene.items():
        ...
        gt_label = [
            os.path.join(args.scene_dir, "labels", "gt", f"{sc}.npz")
            for sc in scene_list
        ]
        gt_label_vis = [
            os.path.join(args.scene_dir, "labels", "gt_vis", f"{sc}.jpg")
            for sc in scene_list
        ]
...
def zip_data(args, zf, geo_info_fn):
    ...
    [
        (
            print(f"zipping {fn}"),
            zf.write(
                os.path.join(args.scene_dir, fn),
...
def get_argparse():
    desc = (
        "This script zip and unzip MVL dataset locally. "
    ...
    parser.add_argument(
        "--labels", action="store_true", help="Different method to zip GT labels."
    )
